const Tournaments = {
  ...
  playCurrent(){
    ...
    for(let i=0;i<t.difficulty.length;i++){
      ...
      matches.push({round:roundNames[i],opponent:this.opponent(),score:this.matchScore(i,won),avg:Math.max(55,p.avg+(Math.random()*8-4)+(p.form-50)*.05).toFixed(1),double:Math.max(12,Math.min(60,p.double+(Math.random()*10-5))).toFixed(1)+"%"});
    ...
    let summary="";
    if(t.access==="qschool"){
      if(rounds>=4){g.tourcardUntilWeek=g.week+104;g.reputation+=3;summary=`Q-School geschafft! ${p.name} erhält eine Tourcard für 2 Jahre.`;UI.log(`Q-School: ${p.name} erspielt sich die Tourcard.`);}
      ...
      g.playerPrize+=prize; g.budget+=Math.floor(prize*.5);
      ...
      else{p.form=Math.min(98,p.form+9);g.reputation+=3;summary=`Turniersieg! Preisgeld: ${UI.euro(prize)}. Dein Anteil: ${UI.euro(Math.floor(prize*.5))}.`;UI.log(`${t.name}: ${p.name} gewinnt das Turnier.`);}
    }
  ...
  report(title,summary,matches){
    let body=`<p class="small">${summary}</p><table><thead><tr><th>Runde</th><th>Gegner</th><th>Ergebnis</th><th>Average</th><th>Doppel</th></tr></thead><tbody>`;
    matches.forEach(m=>body+=`<tr><td>${m.round}</td><td>${m.opponent}</td><td>${m.score}</td><td>${m.avg}</td><td>${m.double}</td></tr>`);
    body+=`</tbody></table>`;
